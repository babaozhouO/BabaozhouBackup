...
Public Class MainForm
    ...
    Private Sub Sftp服务器测试器()
        ...
        If Sftp1开关 Then
            任务列表_上传.Add(1)
            处理单个Sftp服务端_上传文件(Sftp1地址, Sftp1端口, Sftp1用户名, Sftp1密码, "1", 测试文件, "/")
            添加日志("[INFO]正在测试SFTP1服务器", Color.Orange)
        End If
        If Sftp2开关 Then
            任务列表_上传.Add(2)
            处理单个Sftp服务端_上传文件(Sftp2地址, Sftp2端口, Sftp2用户名, Sftp2密码, "2", 测试文件, "/")
            添加日志("[INFO]正在测试SFTP2服务器", Color.Orange)
        End If
        If Sftp3开关 Then
            任务列表_上传.Add(3)
            处理单个Sftp服务端_上传文件(Sftp3地址, Sftp3端口, Sftp3用户名, Sftp3密码, "3", 测试文件, "/")
