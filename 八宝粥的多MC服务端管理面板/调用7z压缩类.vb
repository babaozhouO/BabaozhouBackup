        ...
        Public Sub 执行增量备份(输入目录 As String, 输出目录 As String, Optional 压缩包前缀 As String = "增量备份", Optional 排除文件参数 As String = "")
            ...
            Try
                If 上次备份时间 > DateTime.MinValue Then
                    ...
                    If 压缩器.执行压缩(增量文件临时存放目录(输入目录, 上次备份时间), 输出路径, 生成排除文件参数(排除文件参数)) Then
                        记录备份时间(输出目录, Time)
                ...
                Else
                    ...
                    If 压缩器.执行压缩(输入目录, 输出路径, 生成排除文件参数(排除文件参数)) Then
                        记录备份时间(输出目录, Time)
                        添加日志($"[Success] 备份完成：{压缩器.输出路径}", Color.Green)
                    Else
                        添加日志("[ERROR] 压缩过程返回错误", Color.Red)
            ...
            End Try
        End Sub
        Private Function 生成排除文件参数(排除文件参数 As String) As String
